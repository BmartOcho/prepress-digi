...
def get_illustrator_scripts_path():
    """Attempt to locate the Illustrator Scripts folder via registry or fallback guess."""
    try:
        # Look for installed Illustrator version in registry
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Adobe\Adobe Illustrator")
        ...
        scripts_path = os.path.join(install_path, "Presets", "en_US", "Scripts")
        return scripts_path
    except Exception as e:
        # Fallback (update version folder if needed)
        print("Could not find Illustrator in registry, using fallback.")
        fallback = r"C:\Program Files\Adobe\Adobe Illustrator 2025\Presets\en_US\Scripts"
        return fallback
